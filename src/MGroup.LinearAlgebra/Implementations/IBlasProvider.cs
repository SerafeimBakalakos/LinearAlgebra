...
namespace MGroup.LinearAlgebra.Implementations
{
	...
	public interface IBlasProvider
	{
		...
		void Daxpby(double alpha, double[] x, double beta, double[] y);

		void DaxpbyTo(int n, double alpha, double[] x, int offsetX, int incX, double beta, double[] y, int offsetY, int incY, double[] z, int offsetZ, int incZ);

		void DaxpbyTo(double alpha, double[] x, double beta, double[] y, double[] z);
		...
		void Daxpy(double alpha, double[] x, double[] y);

		void DaxpyTo(int n, double alpha, double[] x, int offsetX, int incX, double[] y, int offsetY, int incY, double[] z, int offsetZ, int incZ);

		void DaxpyTo(double alpha, double[] x, double[] y, double[] z);

		void Dclear(int n, double[] x, int offsetX, int incX);

		void Dcopy(int n, double[] x, int offsetX, int incX, double[] z, int offsetZ, int incZ);
		...
		void Dscal(double alpha, double[] x);

		void DscalTo(int n, double alpha, double[] x, int offsetX, int incX, double[] z, int offsetZ, int incZ);

		void DscalTo(double alpha, double[] x, double[] z);
		#endregion
		...
		void DgemvNoTranspose(int m, int n,
		...
		/// <summary>
		/// Special case of <see cref="DgemvRowMajorNoTranspose(int, int, double, double[], int, int, double[], int, int, double, double[], int, int)"/> for alpha = 1, beta = 0 and tightly packed matrix and vectors. It only performs matrix - vector multiplication.
		/// </summary>
		void DgemvRowMajorNoTranspose(int m, int n, double[] a, double[] x, double[] y);

		void DgemvRowMajorTranspose(int m, int n,
			double alpha, double[] a, int offsetA, int ldA, double[] x, int offsetX, int incX,
			double beta, double[] y, int offsetY, int incY);

		/// <summary>
		/// Special case of <see cref="DgemvRowMajorTranspose(int, int, double, double[], int, int, double[], int, int, double, double[], int, int)"/> for alpha = 1, beta = 0 and tightly packed matrix and vectors. It only performs transposed matrix-vector multiplication.
		/// </summary>
		void DgemvRowMajorTranspose(int m, int n, double[] a, double[] x, double[] y);
